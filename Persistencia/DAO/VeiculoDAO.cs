        ...
        public Veiculo Buscar(long cod)
        {
            try
            {
                using (MySqlCommand comando = _connection.Buscar().CreateCommand())
                {
                    Veiculo veiculo = new Veiculo();
                    comando.CommandType = CommandType.Text;
                    comando.CommandText = "SELECT COD_VEICULO,MARCA,MODELO,ANO_DE_FABRICACAO,KM,CONDICAO,VIDRO_ELETRICO,TRAVA_ELETRICA,AUTOMATICO,QUANTIDADE_PORTAS,DIRECAO_HIDRAULICA,COR,AR_CONDICIONADO,COD_CATEGORIA,STATUS FROM VEICULO WHERE STATUS <> 9 AND COD_VEICULO = @COD_VEICULO;";
